...
async function main() {
  ...
  try {
    ...
    for (const app of apps) {
      ...
      if (app.provider === "bitbucket-datacenter") {
        console.log(`- Bitbucket Data Center callback URL: ${baseUrl}/api/auth/oauth2/callback/${app.slug}`);
        console.log(`- Bitbucket Data Center webhook base: ${baseUrl}/worker/scm-webhook/${app.slug}?connection_token=<token>`);
      }
      if (app.provider === "azure-devops") {
        console.log(`- Azure DevOps callback URL: ${baseUrl}/api/auth/oauth2/callback/azure-devops`);
        console.log(`- Azure DevOps service hook base: ${baseUrl}/worker/scm-webhook/${app.slug}?connection_token=<token>`);
      }
    }
