...
async function main() {
  ...
  try {
    ...
    if (enableAzureDevOps) {
      ...
      const webhookSecret = await askRequired(rl, "Azure DevOps webhook secret");
      const clientId = await askRequired(rl, "Azure DevOps OAuth client ID");
      const clientSecret = await askRequired(rl, "Azure DevOps OAuth client secret");
    ...
    if (enableAzureDevOps) {
      ...
      console.log(`- Azure DevOps callback URL: ${baseUrl}/api/auth/callback/azure-devops`);
      console.log(`- Azure DevOps webhook base: ${baseUrl}/worker/scm-webhook/${azureDevOpsSlug}?connection_token=<token>`);
    }
